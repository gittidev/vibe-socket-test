    ...
    <section class="grid">
      <div class="card">
        ...
        <div v-if="alertItems.length" class="alert">
          <div v-for="(a, i) in alertItems" :key="i" :style="{color: a.severity==='critical' ? 'var(--danger)':'var(--warn)'}">
            ▷ {{ a.message }} ({{ a.severity }})
          </div>
        </div>
        <div v-if="resolvedText" class="alert" :style="{ color: 'var(--primary-accent)' }">해제: {{ resolvedText }}</div>
      </div>
...
function unsubscribe() { if (!selectedId.value) return; socket?.emit('leave_patient', selectedId.value); }

async function loadPatients() {
  const res = await fetch('/api/patients');
  patients.value = await res.json();
  if (patients.value.length && !selectedId.value) { selectedId.value = patients.value[0].id; }
}
...
function fill(dst: Record<K,string>, t?: AlertThresholds | null) { keys.forEach(k => { dst[k] = (t && (t as any)[k] != null) ? String((t as any)[k]) : '' }); }
function clear(dst: Record<K,string>) { keys.forEach(k => dst[k] = ''); }
function collect(src: Record<K,string>) { const out: Record<string, number> = {}; keys.forEach(k => { const v = src[k].trim(); if (v !== '' && !Number.isNaN(Number(v))) (out as any)[k] = Number(v); }); return out; }
...
async function saveDefault() { const body = collect(def.value); await fetch('/api/alert-rules/default', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }); await loadRules(); }
async function saveWard() { if (!wardName.value.trim()) { alert('Ward를 입력하세요'); return; } const body = collect(ward.value); await fetch(`/api/alert-rules/ward/${encodeURIComponent(wardName.value.trim())}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }); await loadRules(); }
async function clearWard() { if (!wardName.value.trim()) { alert('Ward를 입력하세요'); return; } await fetch(`/api/alert-rules/ward/${encodeURIComponent(wardName.value.trim())}`, { method: 'DELETE' }); await loadRules(); }
async function savePatient() { if (!patId.value.trim()) { alert('patientId를 입력하세요'); return; } const body = collect(pat.value); await fetch(`/api/alert-rules/patient/${encodeURIComponent(patId.value.trim())}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }); await loadRules(); }
async function clearPatient() { if (!patId.value.trim()) { alert('patientId를 입력하세요'); return; } await fetch(`/api/alert-rules/patient/${encodeURIComponent(patId.value.trim())}`, { method: 'DELETE' }); await loadRules(); }

onMounted(async () => { connectSocket(); await loadPatients(); await loadRules(); });
...
function colorFor(metric: 'hr'|'sbp'|'spo2'|'temp'|'rr', value?: number) {
  if (value == null) return {} as any; const eff = effective.value as any;
  if (metric === 'hr') { if (eff.hrHigh != null && value > eff.hrHigh) return { color: '#f59e0b' }; if (eff.hrLow != null && value < eff.hrLow) return { color: '#f59e0b' }; }
  if (metric === 'sbp') { if (eff.sbpHigh != null && value > eff.sbpHigh) return { color: '#f59e0b' }; if (eff.sbpLow != null && value < eff.sbpLow) return { color: '#f59e0b' }; }
  if (metric === 'spo2') { if (eff.spo2Low != null && value < eff.spo2Low) return { color: '#f59e0b' }; }
  if (metric === 'temp') { if (eff.tempHigh != null && value >= eff.tempHigh) return { color: '#f59e0b' }; }
  if (metric === 'rr') { if (eff.rrHigh != null && value > eff.rrHigh) return { color: '#f59e0b' }; if (eff.rrLow != null && value < eff.rrLow) return { color: '#f59e0b' }; }
  return { color: '#e5e7eb' };
}
</script>
...
select, input { background: var(--secondary-accent); color: var(--text); border: 1px solid var(--card-border); border-radius: 8px; padding: 8px 10px; }
.grid { display: grid; grid-template-columns: repeat(2, minmax(0,1fr)); gap: 16px; }
.kv { display: grid; grid-template-columns: 120px 1fr; gap: 8px; margin: 4px 0; }
.vital { font-family: ui-monospace, SFMono-Regular, Menlo, monospace; }
.alert { font-weight: 600; margin-top: 8px; }
footer { margin-top: 24px; color: var(--text); font-size: 12px; text-align: center; }
